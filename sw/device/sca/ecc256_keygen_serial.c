...
static void ecc256_ecdsa_secret_keygen_batch(const uint8_t *data,
  ...
  for (uint32_t i = 0; i < num_traces; ++i) {
    ...
    // Read results.
    SS_CHECK(otbn_dmem_read(kEcc256SeedNumWords, kOtbnVarD0, d0_batch) ==
             kOtbnErrorOk);
    SS_CHECK(otbn_dmem_read(kEcc256SeedNumWords, kOtbnVarD1, d1_batch) ==
             kOtbnErrorOk);

    // // XOR of both shares to include both into the batch digest
    // for (uint32_t j = 0; j < kEcc256SeedNumWords; ++j) {
    //   out[j] = d0_batch[j] ^ d1_batch[j];
    // }
