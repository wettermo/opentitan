 ...
 * @return OK or error.
 */
static status_t uint_to_bytes(size_t num, uint8_t *bytestring) {
...
otcrypto_status_t otcrypto_kdf_hmac_ctr(
  ...
  uint8_t required_len_bytestr[4];
  uint_to_bytes(required_byte_len * 8, required_len_bytestr);
  memcpy(input_data + 4 + kdf_label.len + 1 + kdf_context.len,
         required_len_bytestr, 4);
  ...
  for (size_t i = 0; i < num_iterations; i++) {
    uint_to_bytes(i + 1, counter_bytestr);
    memcpy(input_data, counter_bytestr, 4);
