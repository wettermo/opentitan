    ...
    if (cmd_phase_write) begin
      case (csr_name)
        ...
        "control": begin
          ...
          spi_ctrl_reg.tx_watermark = bit'(get_field_val(ral.control.tx_watermark, item.a_data));
          spi_ctrl_reg.rx_watermark = bit'(get_field_val(ral.control.rx_watermark, item.a_data));
          if (cfg.en_cov) begin
        ...
        "intr_state": begin
          ...
          do_read_check = 1'b0;
          if (cfg.en_cov) begin
          cov.interrupt_cg.sample( spi_intr_state_reg, spi_intr_enable_reg, spi_intr_test_reg);
          end
        end
        "intr_enable": begin
        ...
        "error_status": begin
          ...
          spi_error_status_reg.cmdbusy       = bit'(get_field_val(ral.error_status.cmdbusy,
                                                                  item.a_data));
          if (cfg.en_cov) begin
          cov.error_status_cg.sample(spi_error_status_reg);
          end
        end
        "event_enable": begin
