...
module aes_syn_wrap_tb #(
  ...
  always_ff @(posedge clk_i or negedge rst_ni) begin : tb_ctrl
    ...
    if (init_mark) begin
      ...
      fd = $fopen("../ot-sca/cw/cw305/sifa_aes_syn_output_verilator_sim.csv", "w");
    ...
    if (read_next_line) begin
      ...
      if (fgets_ret == 0) begin
        ...
        // somehow this line is necessary for the TB to read the next line of fd_2
        // -> if commented out, next line reading fails
        // maybe this could be caused by some compiler optimization
        $display("fd(out): %d, fd_2(in): %d", fd, fd_2);
