...
module aes_sbox_faulted import aes_pkg::*;
#(
  ...
  if (!SBoxMasked) begin : gen_sbox_unmasked
    ...
    logic [7:0] data_sin;
    logic [7:0] data_sout;
    logic [7:0] data_magic;

    assign data_sin = data_i;
    assign data_magic = {2'b0, data_sout[5:0]};
    assign data_o = fault_en ? data_magic : data_sout;
    ...
    if (SecSBoxImpl == SBoxImplDom) begin : gen_sbox_dom
      ...
      aes_sbox_dom_faulted u_aes_sbox (
        .clk_i      ( clk_i       ),
        .rst_ni     ( rst_ni      ),
        .fault_en   ( fault_en),
        .en_i       ( en_i        ),
