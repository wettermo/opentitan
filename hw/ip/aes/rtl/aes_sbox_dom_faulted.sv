...
// See Fig. 2 in [1].
module aes_dom_indep_mul_gf2pn_faulted #(
  ...
  import aes_sbox_canright_pkg::*;
    ...
    //fault one mask
  logic [NPower-1:0] fault_mask;
  assign fault_mask = {3'b111, ~fault_en};      
  //assign fault_mask = {2'b11, ~fault_en, 1'b1}; 
  //assign fault_mask = {1'b1, ~fault_en, 2'b11};
  //assign fault_mask = {~fault_en, 3'b111}; 
...
// See Formula 12 in [1].
module aes_dom_dep_mul_gf2pn_faulted #(
  parameter int unsigned NPower      = 4,
  parameter bit          Pipeline    = 1'b0,
  ...
  //fault one mask
  logic [NPower-1:0] fault_mask;
  //assign fault_mask = {{(NPower-1){1'b1}}, ~fault_en};
  assign fault_mask = {(NPower){~fault_en}};
  //assign fault_mask = {2'b11, ~fault_en, 1'b1}; 
  //assign fault_mask = {1'b1, ~fault_en, 2'b11};
  //assign fault_mask = {~fault_en, 3'b111}; 
  ...
  if (PreDomIndep == 1'b1) begin : gen_pre_dom_indep
    ...
    assign a_q = (axz0_z1_q & fault_mask) ^ a_mul_ax_b_buf;
    assign b_q = (bxz0_z1_q & fault_mask) ^ b_mul_bx_b_buf;
...
module aes_dom_inverse_gf2p4_faulted #(
...
module aes_dom_inverse_gf2p8_faulted #(
  parameter bit PipelineMul = 1'b1
) (
  input  logic        clk_i,
  input  logic        rst_ni,
  ...
  input  prd_in_faulted_t     prd_i,   // pseudo-random data, e.g. for intermediate masks
  output logic  [7:0] a_y_inv, // output data masked by b_y_inv
  output logic  [7:0] b_y_inv, // output mask
  output prd_out_faulted_t    prd_o    // pseudo-random data, e.g. for use in another S-Box instance
);
  ...
  aes_dom_inverse_gf2p4_faulted #(
    .PipelineMul ( PipelineMul )
  ) u_aes_dom_inverse_gf2p4_faulted (
    .clk_i       ( clk_i       ),
    .rst_ni      ( rst_ni      ),
    .fault_en    ( fault_en    ),
    .we_i        ( we_i[2:1]   ),
  ...
  aes_dom_indep_mul_gf2pn_faulted #(
    .NPower   ( 4           ),
    .Pipeline ( PipelineMul )
  ) u_aes_dom_mul_theta_y1_faulted (
    .clk_i  ( clk_i            ),
    .rst_ni ( rst_ni           ),
    .fault_en (1'b0),
    .we_i   ( we_i[3]          ),
  ...
  aes_dom_indep_mul_gf2pn_faulted #(
    .NPower   ( 4           ),
    .Pipeline ( PipelineMul )
  ) u_aes_dom_mul_theta_y0_faulted (
    .clk_i  ( clk_i            ),
    .rst_ni ( rst_ni           ),
    .fault_en (1'b0),
    .we_i   ( we_i[3]          ),
...
#(
  ...
  prd_in_faulted_t    in_prd;
  prd_out_faulted_t   out_prd;
