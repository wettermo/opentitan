...
#(
  ...
  // DUT
  aes aes (
    .clk_i           (clk_i),                         // logic -> wire
    .rst_ni          (rst_ni),                        // logic -> wire
    .rst_shadowed_ni (rst_ni),                        // logic -> wire
    .idle_o          (unused_idle),                   // logic [3:0] -> wire [3:0]
    .lc_escalate_en_i(lc_ctrl_pkg::Off),              // lc_ctrl_pkg::Off = 4'b1010 -> wire [3:0]
    .clk_edn_i       (clk_i),                         // logic -> wire
    .rst_edn_ni      (rst_ni),                        // logic -> wire
    .edn_o           (edn_req),                       // logic -> wire
    .edn_i           ({edn_req, 1'b1, entropy_mask}), // {logic, 1'b1, logic [31:0]} -> wire [33:0]
    .keymgr_key_i    (keymgr_key),                    // {logic, logic [1:0][255:0]} -> wire [512:0]
    .tl_i            (h2d_intg),                      // ? -> wire [108:0]
    .tl_o            (d2h),                           // ? -> wire [65:0]
    .alert_rx_i      (alert_rx),                      // {logic, logic, logic, logic}{logic, logic, logic, logic} -> wire [7:0]
    .alert_tx_o      (alert_tx)                       // {logic, logic}{logic, logic} -> wire [3:0]
  );
